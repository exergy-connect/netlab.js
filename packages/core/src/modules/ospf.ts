...
export function node_post_transform(node: Node, topology: Topology, _ctx: TransformContext): void {
  ...
  const hasV4 = (node.interfaces ?? []).some((i) => i.ipv4) || Boolean((node.loopback as JsonObject)?.ipv4);
  ospf.af = { ipv4: hasV4, ipv6: false };

  // p2p interfaces → point-to-point network type
